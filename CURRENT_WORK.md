...
- Production follow-up: fixed edited items saving `discountType: 'CASH'`, which the order schema rejected because it requires the numeric enum (`1` percentage, `2` cash). Edited items now save `discountType: 2`, catalog price loading handles both numeric and legacy string representations, and conversion normalizes already-saved v1 records so they work without another edit. The editor asset version is bumped to `v=2` to invalidate the broken browser cache. Verification: editor scripts parse and match byte-for-byte; backend tests pass (3 suites/12 tests, including legacy conversion coverage); backend build and `git diff --check` pass.
- Admin-path correction: the first compatibility pass normalized `newOrderMake()`, but the incomplete-list Add Order button calls `addOrderAdmin()`, which constructed the Mongoose document directly. Normalization now runs inside `addOrderAdmin()` before document construction, and both creation paths share the same helper. An exact-path regression test verifies the order model receives numeric `discountType`; backend tests pass (3 suites/13 tests) and the backend build passes.
...
- Deployment TODO: rebuild/restart the backend so the authenticated endpoint is primary in production; the compatibility fallback below keeps order creation working if static assets arrive first.
- Production compatibility fix (2026-07-21): the live widget encountered `Cannot POST /api/order/add-assisted` because the static asset was deployed before the running backend exposed the new route. The widget now retries through the existing `/api/order/add-order-by-anonymous` flow only when the authenticated endpoint returns HTTP 404; 401/403, validation failures, and server errors never bypass authentication. Both script tags were bumped to `v=2` to invalidate the broken browser cache. Mocked request assertions verified the 404 fallback and confirmed that a 401 makes exactly one request with no fallback.
...
- Replaced the bulky inline homepage book cards with a reusable modern-minimal card: quieter discount tab, compact edge-to-edge cover stage, aligned price rail, and accessible wishlist behavior.
- Refined the cards again after visual review: removed the ambiguous icon-only cart control, added a full-width `কার্টে যোগ করুন` action with visible loading/success/error copy, reduced the cover-stage height, tightened the content rhythm, and aligned desktop carousel cards to a consistent height.
- Enlarged product artwork to use the full available media stage with only a small responsive inset; covers remain uncropped through `object-fit: contain` while eliminating the excessive empty space around portrait images.
...
- Checks: frontend production build passed; browser checks confirmed the full CTA label remains visible and single-line, all sampled covers load, controls remain 44px tall, desktop cards align, and no horizontal overflow occurs at 320/375/414/768/1280px. `npx tsc --noEmit` remains blocked only by the pre-existing unsupported `eslint` property in `next.config.ts`; `npm run lint` remains blocked because Next.js 16 no longer supports the configured `next lint` command.
